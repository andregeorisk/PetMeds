...
async function loadRemoteData() {
  ...
  db.treatments = treatments.map(treatment => ({ ...treatment, familyId: treatment.family_id, petId: treatment.pet_id, medId: treatment.medication_id, intervalDays: treatment.interval_days, intervalValue: treatment.interval_value || treatment.interval_days || 1, intervalUnit: treatment.interval_unit || 'day', startDate: treatment.start_date || treatment.created_at.substring(0, 10), durationType: treatment.duration_type, endDate: treatment.end_date, createdAt: treatment.created_at, times: treatment.times.map(time => time.substring(0, 5)) }));
...
async function saveTreatment(event) {
  ...
  const values = { pet_id: document.getElementById('treat-pet').value, medication_id: document.getElementById('treat-med').value, dosage: document.getElementById('treat-dosage').value.trim(), times, start_date: startDate, interval_value: intervalValue, interval_unit: intervalUnit, interval_days: intervalDays, duration_type: durationType, end_date: endDate };
  try {
    if (id) {
